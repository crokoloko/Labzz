...
# ==========================================
def fmt_num(val):
    """Formatta i numeri in modo che 10.0 diventi '10' e 10.5 rimanga '10.5'."""
    val = float(val)
    ...
    if prodotti_df.empty:
        ...
        if tipo_operazione == "Vendita":
            ...
            if lotti_disponibili.empty:
                ...
                st.info(f"Disponibilità attuale per **{prod_nome}**: **{fmt_num(qta_tot_disp)} g**")
                ...
                with st.form("form_vendita"):
                    ...
                    with col1:
                        ...
                        prezzo_vendita_unitario = st.number_input("Prezzo al grammo (€/g)", min_value=0.5, value=float(prod_row['valore_mercato_unitario']), step=0.5, format="%.1f")
                    
                    with col2:
                        totale_vendita = quantita_vendita * prezzo_vendita_unitario
                        st.metric("Totale Incasso Previsto", f"€ {fmt_num(totale_vendita)}")
                        note = st.text_input("Note (Opzionale)")

                    if st.form_submit_button("Vendita"):
                        if quantita_vendita > qta_tot_disp:
                            st.error(f"Quantità inserita ({fmt_num(quantita_vendita)} g) superiore alla disponibilità ({fmt_num(qta_tot_disp)} g).")
                        else:
                            conn = get_connection()
                            cursor = conn.cursor()
                            ...
                            conn.commit()
                            conn.close()
                            st.success(f"✅ Vendita registrata! Incasso: €{fmt_num(ricavo_totale)} | Margine: €{fmt_num(margine)}")
                            st.rerun()
            ...
            with st.form("form_carico"):
                ...
                with col1:
                    ...
                    costo_unitario = st.number_input("Costo d'Acquisto al grammo (€/g)", min_value=0.5, value=1.0, step=0.5, format="%.1f")
            ...
            if lotti_df.empty:
                ...
                with st.form("form_xme"):
                    ...
                    opzioni_lotto = {f"{r['prodotto']} - Lotto: {r['codice_lotto']} (Disp: {fmt_num(r['quantita_attuale'])} g)": r['id'] for _, r in lotti_df.iterrows()}
                    ...
                    if st.form_submit_button("Conferma XME"):
                        ...
                        conn.commit()
                        conn.close()
                        st.warning(f"Operazione XME registrata! Valore associato: €{fmt_num(costo_perdita)}")
                        st.rerun()
...
with tab2:
    ...
    with col_a:
        with st.expander("➕ **Aggiungi Nuovo Prodotto**", expanded=False):
            with st.form("form_nuovo_prodotto"):
                ...
                costo_u_init = st.number_input("Costo d'Acquisto al grammo (€/g)", min_value=0.5, value=1.0, step=0.5, format="%.1f")
                prezzo_v_init = st.number_input("Prezzo di Vendita al grammo (€/g)", min_value=0.5, value=2.0, step=0.5, format="%.1f")
        ...
        with st.expander("⚡ **Aggiorna Quantità / Prezzo Prodotto Esistente**", expanded=True):
            ...
            if not prodotti_df.empty:
                ...
                with st.form("form_rettifica_diretta"):
                    ...
                    nuovo_costo_grammo = st.number_input("Costo d'Acquisto Unitario (€/g)", min_value=0.5, value=float(costo_u_att if costo_u_att > 0 else 1.0), step=0.5, format="%.1f")
                    nuovo_prezzo_grammo = st.number_input("Prezzo di Vendita Unitario (€/g)", min_value=0.5, value=float(p_row['valore_mercato_unitario']), step=0.5, format="%.1f")
    ...
    df_stato_disp = df_stato.copy()
    df_stato_disp['qta_disponibile'] = df_stato_disp['qta_disponibile'].apply(fmt_num)
    df_stato_disp['costo_medio_ponderato'] = df_stato_disp['costo_medio_ponderato'].apply(fmt_num)
    df_stato_disp['valore_mercato_unitario'] = df_stato_disp['valore_mercato_unitario'].apply(fmt_num)
    df_stato_disp['valore_totale_costo'] = df_stato_disp['valore_totale_costo'].apply(fmt_num)
    df_stato_disp['valore_totale_mercato'] = df_stato_disp['valore_totale_mercato'].apply(fmt_num)
...
with tab3:
    ...
    col1.metric("Valore Magazzino (Costo)", f"€ {fmt_num(df_stato['valore_totale_costo'].sum())}")
    col2.metric("Valore Magazzino (Vendita)", f"€ {fmt_num(df_stato['valore_totale_mercato'].sum())}")
    col3.metric("Incasso Totale Vendite", f"€ {fmt_num(df_stato['incasso_totale'].sum())}")
    col4.metric("Margine Netto Effettivo", f"€ {fmt_num(df_stato['margine_totale'].sum())}")
...
with tab4:
    ...
    if movimenti_df.empty:
        ...
        df_filtrato['quantita'] = df_filtrato['quantita'].apply(fmt_num)
        df_filtrato['prezzo_unitario'] = df_filtrato['prezzo_unitario'].apply(fmt_num)
        df_filtrato['ricavo_totale'] = df_filtrato['ricavo_totale'].apply(fmt_num)
        df_filtrato['costo_totale'] = df_filtrato['costo_totale'].apply(fmt_num)
        df_filtrato['margine'] = df_filtrato['margine'].apply(fmt_num)
